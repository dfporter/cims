	...
	foreach my $clust (@$clusters)
	{
		...
		if ($weight && $keepMaxScore)
		{
			...
			$clust = \@clustSorted;
                ...
                #print "randomBarcode = ", $randomBarcode; #dfporter
		if ($randomBarcode)
		{
			...
			{
                                #print "if $randomBarcode: divide tags according to rand link, tag in clust: ", $tag; #dfporters
			...
			else
			{
				...
				foreach my $linker (keys %tagsInClust)
				{
					...
					$representativeTag->{"name"} .= "[o=" . $tagsInClustWithSameLinker->{"score"} . "]" unless $keepTagName;
                                        #print "No EM: divide tags according to rand link, tag in clust: "; #dfporters
					print $fout bedToLine ($representativeTag), "\n";
				}
			}
		}
		...
		{	#no barcode
                        #print "No barcode, clust = ", $clust->[0]->{'name'}, "\nstrand=", $clust->[0]->{'strand'}, "\n"; #dfporter
