...
if ($trackPosition)
{
	...
	$cmd = "paste $mutationBedFile $tmpFile | awk '{if(\$6 ==\"+\") {print \$5;} else {print \$9-\$8\-\$5-1}}' > $mutationPositionFile";
...
my $cmd = "perl $cmdDir/tag2cluster.pl  -s  -maxgap \"-1\" $bigFlag $verboseFlag $mutationBedFile $mutationClusterFile";
$cmd .= " -big" if $big;
system ($cmd);
...
for (my $i = 0; $i < $numIter; $i++)
{
	...
	$cmd = "perl $cmdDir/simulateCLIPmismatch.pl -n $totalMutations $trackPosFlag -srand $rand_seed $tagBedFile $randomMutationBedFile";
	...
	$cmd = "perl $cmdDir/tag2cluster.pl  -s  -maxgap \"-1\" $randomMutationBedFile $randomMutationClusterFile";
